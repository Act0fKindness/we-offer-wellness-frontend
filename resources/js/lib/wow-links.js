// Random underline effect for links with [data-draw-line]
// Lightweight reimplementation without GSAP DrawSVG

function decorateSVG(svgEl) {
  svgEl.setAttribute('class', 'drawline-svg')
  svgEl.setAttribute('preserveAspectRatio', 'none')
  svgEl.querySelectorAll('path').forEach((p) => p.setAttribute('stroke', 'currentColor'))
}
...
function animatePath(path, toOffset, duration = 500) {
  const length = path.getTotalLength()
  const start = performance.now()
  const from = parseFloat(path.style.strokeDashoffset || String(length))
  ...
  function step(now) {
    ...
    path.style.strokeDashoffset = String(from + delta * eased)
...
export function initDrawRandomUnderline() {
  ...
  document.querySelectorAll('[data-draw-line]').forEach((container) => {
    ...
    if (container.classList.contains('link-wow--nav')) return
    ...
    container.addEventListener('mouseenter', () => {
      if (nextIndex === null) nextIndex = Math.floor(Math.random() * svgVariants.length)
    ...
    container.addEventListener('mouseleave', () => {
      ...
      if (!path) return
      const len = path.getTotalLength()
      animatePath(path, -len, 500)
      clearTimeout(outTimer)
      outTimer = setTimeout(() => { box.innerHTML = '' }, 520)
    })
