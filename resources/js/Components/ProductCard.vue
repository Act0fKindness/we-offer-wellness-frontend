...
function locations(product) {
  const source = Array.isArray(product?.locations) ? product.locations : [product?.matched_location_label || product?.location || product?.location_name || product?.venue]
...
function hasOnline(product) {
  return text(product?.format).toLowerCase().includes('online')
