...
const props = defineProps({
  ...
  badges: { type: Array, default: () => [
    { icon: 'bi-gem', text: 'Preferred' },
    { icon: 'bi-lightning-charge', text: 'Instant booking' },
  ] },
  typeLabel: { type: String, default: 'Therapy • Online or In-person' },
  title: { type: String, required: true },
  rating: { type: [String, Number], default: '4.9' },
  hasReviews: { type: Boolean, default: true },
  reviewsText: { type: String, default: 'Excellent • 128 reviews' },
  meta: { type: Array, default: () => [
    { icon: 'bi-camera-video', text: 'Online available' },
    { icon: 'bi-geo-alt', text: 'Kent, England' },
    { icon: 'bi-alarm', text: '45–60 mins' },
    { icon: 'bi-people', text: '1–2 persons' },
  ] },
  policyText: { type: String, default: 'Free cancellation 24h' },
  price: { type: String, default: '£48.00' },
  priceNote: { type: String, default: '/ from' },
  ctaLabel: { type: String, default: 'Book Therapy' },
...
function addToCart(e){
  ...
  try {
    ...
    if (btn) { btn.classList.add('btn-press'); setTimeout(()=>btn.classList.remove('btn-press'), 160) }
    window.dispatchEvent(new CustomEvent('wow:add-to-cart', { detail: { id: item.id } }))
  } catch {}
}
...
<template>
  <article :class="['wow-card', props.size === 'md' ? 'md' : '', 'therapy']" data-type="therapy">
    <div class="wow-media">
      ...
      <div v-if="showQuickView" class="quick-view">
