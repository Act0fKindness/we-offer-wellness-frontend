...
@once
  ...
  .wow-therapy-card-scope .therapy-card{ background: var(--card); border: var(--borderW) solid rgba(16,24,40,.18); border-radius: var(--radius); overflow:hidden; position:relative; min-width:0; height: var(--cardH); display:flex; flex-direction:column }
  ...
  .wow-therapy-card-scope .badge--warm.badge--long{ background:#fff2e0; color:#5a350a; }
...
/* Ensure popover can overflow the content area */
  .wow-therapy-card-scope .content-top{ overflow: visible; padding:15px !important; }
  .wow-therapy-card-scope .content-top.wow-content-top{ display:flex; flex-direction:column; height:100%; }
  ...
  .wow-therapy-card-scope .loc-popover{ position:absolute; top:calc(100% + 10px); left:50%; width:260px; max-width:calc(100vw - 32px); background:#fff; border:1px solid rgba(15,24,40,.1); border-radius:16px; box-shadow:0 28px 65px rgba(11,18,32,.25); padding:16px; opacity:0; visibility:hidden; transform:translate(calc(-50% + var(--loc-popover-shift, 0px)), -6px); transition:opacity .18s ease, transform .18s ease; z-index:40; pointer-events:none; }
